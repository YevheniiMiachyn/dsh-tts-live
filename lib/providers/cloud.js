...
import { markPiece } from "../latency.js"
...
export function makeCloudProviders(deps, req, withCloudTimeout) {
  ...
  // LOCAL FORK: latency instrumentation - the voice turn this synthesis belongs
  // to, so T1 (request issued) and T2 (audio complete) land on the right piece.
  const { traceId, piece } = req
  ...
  function openaiCompatible(key) {
    ...
    return async function speak() {
      ...
      const { signal, cleanup } = withCloudTimeout()
      try {
        // LOCAL FORK: latency instrumentation - T1 is the real synthesis HTTP
        // request; T2 is the instant its audio is fully in memory and playable.
        markPiece(traceId, piece, 'T1', { provider: key })
      ...
      const audio = asBuffer(Buffer.from(await res.arrayBuffer()))
      markPiece(traceId, piece, 'T2', { provider: key, bytes: audio.length })
