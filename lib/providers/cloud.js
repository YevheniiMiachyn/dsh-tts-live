...
/**
 * LOCAL FORK (PCM stage): progressive PCM synthesis for an OpenAI-compatible
 * endpoint.
 *
 * This is the same request as `openaiCompatible(key)` with one difference that
 * matters: `response_format: "pcm"` and the body is READ AS IT ARRIVES instead of
 * `await res.arrayBuffer()`. On qwentts.cpp that switches the whole server-side
 * pipeline as well — the codec decode becomes the per-frame streaming path and
 * the response becomes a chunked `audio/pcm` body whose first bytes exist ~50 ms
 * after the request rather than after the whole utterance (~570 ms measured).
 *
 * The chain semantics, the credential lookup, the timeout wrapper and the
 * sampling fields are deliberately identical to the buffered provider so that a
 * piece sounds the same whichever transport carried it.
 *
 * Returns a function per provider key. `onChunk(Uint8Array, seq)` may return
 * false to stop reading, which cancels the response body and therefore aborts
 * synthesis on the server.
 */
export function makePcmStreamers(deps, req, withCloudTimeout) {
  const { resolveKey, fetchImpl, cfg } = deps
  const { text, models, voices } = req

  function streamer(key) {
    return async function streamPcm(onChunk) {
      const spec = OPENAI_COMPATIBLE[key]
      const keyEnv = (cfg[key + 'KeyEnv'] || spec.keyEnv)
      const token = await resolveKey(keyEnv)
      if (!token) return { ok: false, provider: key, reason: `no ${keyEnv}` }
      ...
      try {
        ...
        if (!res.body || typeof res.body.getReader !== 'function') {
          throw new Error(`${key} returned no readable stream body`)
        }
        ...
        for (;;) {
          ...
          const keep = await onChunk(value, chunks)
          chunks++
          if (keep === false) { try { await reader.cancel() } catch { /* already gone */ } break }
        }
        return { ok: bytes > 0, provider: key, bytes, chunks, reason: bytes ? '' : 'empty audio' }
      } finally {
        cleanup()
      }
    }
  ...
  const out = {}
  for (const key of Object.keys(OPENAI_COMPATIBLE)) out[key] = streamer(key)
  return out
}
