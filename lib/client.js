...
`
      ...
      return {
        ...
        handleAbort(meta) {
          ...
          // LOCAL FORK (0.4.16-local.14): an abort can be the last event a reply
          // ever has. Releasing the stream removes it from `streams`, so nothing
          // is left to reach the `finished` branch in pump() — and that branch is
          // the only other place the end-of-playback notification is armed. Without
          // this, `speaking` stayed true for the rest of the page's life and the
          // dock's controls never retracted after a cancelled reply.
          if (streams.size === 0) scheduleIdle(playbackEndCtx)
          pump()
        },
    ...
    function pcmEvent(kind, data) {
      try {
        // GRAPH-WIDE EVENTS FIRST. `idle` and `stop` describe the whole audio graph,
        // not one stream, so they carry NO streamId — and the per-stream lookup
        // below returns early when there is no record. Handling them after it meant
        // the branch was unreachable: `emit('idle', {})` was swallowed every time.
        //
        // Reported from real use as "the controls show up and then never disappear
        // again". The swallow itself was invisible in local.12 — nothing consulted
        // `speaking` — and became visible in local.13, when the dock started using
        // it to decide whether to render at all. The same missing call also left
        // `dsh:tts:stop` undispatched, so dsh-voice's barge-in gate went on
        // believing the assistant was still talking after she had stopped.
        if (kind === 'idle' || kind === 'stop') { announceSpeech(false); return }
    ...
    function bargeInNow() {
      ...
      // LOCAL FORK (0.4.16-local.14): scheduled-but-unplayed PCM has to count.
      //
      // `activeStreams` is the number of streams still RECEIVING BYTES, and the
      // producer runs several times faster than real time — so by the moment the
      // user starts talking, that count is usually already 0 while a second or more
      // of speech sits booked into the AudioContext. This guard then returned
      // early, `stopPlayback()` never ran, and the tail kept playing over the user.
      // dsh-voice's own barge-in could not save it either: `triggerBargeIn()` pauses
      // <audio> elements, and progressive PCM has none.
      //
      // `playing` is the term that answers "is there still sound to come"
      // (pcm-player.js `playingRemainingMs`), so the stop is decided the same way
      // the dock decides whether she is speaking.
      if (!player.audio && !player.queue.length && !player.busy &&
          pcmStreaming.activeStreams === 0 && !pcmStreaming.playing) return
      stopPlayback()
    }
...
async function pollPending() {
      ...
      try {
        ...
        // LOCAL FORK (0.4.16-local.14): silent mode has to reach the PCM graph.
        //
        // The host honours it the instant it changes — no new synthesis, and
        // in-flight streams aborted (index.js `setSpeechMuted` → `silenceNow`). But
        // aborting a stream stops the BYTES; it cannot touch blocks already booked
        // into the AudioContext, and this transport deliberately schedules far
        // ahead of the playhead. "Nobody is listening" has to be true now, not
        // after whatever the scheduler already handed to the graph.
        if (!player.enabled && wasEnabled) pcmStop('silence')
