...
// LOCAL FORK: live sentence-level flush options (see LIVE-SENTENCE.md).
import { liveOptions } from './live.js'
import { TIMEOUT_REASON } from './breaker.js'
...
export function registerHttpRoutes(ctx, api) {
  const {
    ...
    queueSnapshot,
    // LOCAL FORK (0.4.16-local.6): silent mode accessors.
    isSpeechMuted,
    setSpeechMuted,
    // LOCAL FORK (PCM stage): progressive transport.
    pcmTransport,
    pcmConfig,
  } = api
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const cfg = live()
      const liveCfg = liveOptions ? liveOptions(cfg) : undefined
      writeJson(res, 200, {
        ok: true,
        speakReplies: cfg.speakReplies,
        // LOCAL FORK (0.4.16-local.6): silent mode. `speechMuted` is the runtime
        // state (silent because nobody is listening) and is never persisted;
        // `speechSuppressed` is the question the browser actually asks — "will
        // anything be spoken at all" — so the two reasons for silence cannot be
        // mistaken for one another.
        speechMuted: typeof isSpeechMuted === 'function' ? !!isSpeechMuted() : false,
        speechSuppressed: (typeof isSpeechMuted === 'function' ? !!isSpeechMuted() : false) || !cfg.speakReplies,
        ...
        chain: cfg.chain,
        roles: cfg.roles,
        // LOCAL FORK: live sentence-level flush. The browser uses
        // `live.pollMs` as its pending-poll interval while `live.enabled`.
        ...(liveCfg === undefined ? {} : {
          live: {
            enabled: liveCfg.enabled,
            pollMs: liveCfg.pollMs,
            minCharsFirst: liveCfg.minCharsFirst,
            minChars: liveCfg.minChars,
            maxChars: liveCfg.maxChars,
          },
        }),
        providers: PROVIDER_KEYS,
        sseClients: streamSubscriberCount(),
        // LOCAL FORK (0.4.16-local.2): queue state. A benchmark harness uses this
        // as a drain barrier — a run is finished only when nothing is reserved
        // and no synthesis is still in flight.
        queue: typeof queueSnapshot === 'function' ? queueSnapshot() : undefined,
        breaker: typeof breakerSnapshot === 'function' ? breakerSnapshot() : [],
        // LOCAL FORK (PCM stage): the browser decides whether to run the PCM
        // player from this block alone. `enabled` is the whole feature switch; the
        // other fields are the parameters it needs to start.
        ...(typeof pcmConfig !== 'function' ? {} : (() => {
          const pcm = pcmConfig()
          return {
            pcm: {
              enabled: pcm.experimental === true && !!pcm.provider,
              experimental: pcm.experimental === true,
              startupMs: pcm.startupMs,
              telemetry: pcm.telemetry === true,
              provider: pcm.provider || '',
              active: typeof pcmTransport === 'object' && pcmTransport !== null
                ? (typeof pcmTransport.activeCount === 'function' ? pcmTransport.activeCount() : 0)
                : 0,
            },
          }
        })()),
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      if (req.method === 'GET') {
        writeJson(res, 200, { ok: true, total: stats.total, cacheHits: stats.cacheHits, errors: stats.errors, queueEvictions: stats.queueEvictions || 0, providers: stats.providers, breaker: typeof breakerSnapshot === 'function' ? breakerSnapshot() : [] })
        return
      }
      if (req.method !== 'DELETE') {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // LOCAL FORK (0.4.16-local.7): same reasoning as the agent path — a timeout
      // must remain distinguishable from a cancelled turn.
  ...
  // LOCAL FORK: barge-in. The browser fires this when the microphone opens
  // (dsh-voice's `dsh-voice:speaking`). It aborts in-flight synthesis and empties
  // the pending queue, so a reply the user talked over cannot keep speaking
  // afterwards. Without it, pieces still synthesizing settle into the queue and
  // are collected by the next poll — i.e. the agent talks over the user.
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-tts/bargein',
    handler: async (req, res) => {
      if (req.method !== 'POST') { writeJson(res, 405, { ok: false, error: { code: 'method', message: 'POST only' } }); return }
      let payload = {}
      try { payload = JSON.parse((await readBody(req, 8 * 1024)).toString('utf8') || '{}') } catch { /* empty body is fine */ }
      const sid = payload && payload.sessionId ? String(payload.sessionId) : undefined
      try {
        if (liveEngine && typeof liveEngine.abortAll === 'function') liveEngine.abortAll(sid)
        else if (typeof dropPendingForSession === 'function') dropPendingForSession(sid)
        writeJson(res, 200, { ok: true, dropped: pending.length })
      } catch (error) {
        writeJson(res, 500, { ok: false, error: { code: 'bargein', message: String(error && error.message || error) } })
      }
    },
  }), 'dsh-tts: /bargein route')

  // LOCAL FORK (0.4.16-local.6): silent-mode toggle for callers outside the agent —
  // a script, a shortcut, or a UI button. In memory only: nothing is written to
  // settings.yaml, and a restart clears it. GET reports, POST sets.
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-tts/silence',
    handler: async (req, res) => {
      const current = typeof isSpeechMuted === 'function' ? !!isSpeechMuted() : false
      if (req.method === 'GET') { writeJson(res, 200, { ok: true, speechMuted: current }); return }
      if (req.method !== 'POST') {
        writeJson(res, 405, { ok: false, error: { code: 'method', message: 'GET or POST' } })
        return
      }
      if (!isTrustedSettingsRequest(req)) {
        writeJson(res, 403, { ok: false, error: { code: 'forbidden', message: 'dsh-tts: silence writes are same-origin only' } })
        return
      }
      if (typeof setSpeechMuted !== 'function') {
        writeJson(res, 503, { ok: false, error: { code: 'unavailable', message: 'silent mode is not wired' } })
        return
      }
      let payload = {}
      try { payload = JSON.parse((await readBody(req, 8 * 1024)).toString('utf8') || '{}') } catch { /* empty body is fine */ }
      if (typeof payload.muted !== 'boolean') {
        writeJson(res, 400, { ok: false, error: { code: 'body', message: 'muted must be a boolean' } })
        return
      }
      const out = setSpeechMuted(payload.muted, typeof payload.reason === 'string' && payload.reason ? payload.reason : 'http')
      writeJson(res, 200, { ok: true, speechMuted: out.muted, changed: out.changed })
    },
  }), 'dsh-tts: /silence route')

  // LOCAL FORK (PCM stage): the measuring instrument, in one place.
  //
  // The browser owns P4..P7 and the host owns P0..P3, and neither can see the
  // other's clock. The browser posts its half here, keyed by streamId; the host
  // merges it into the record it already holds and returns the whole end-to-end
  // picture. DELETE clears the ring.
  //
  // The records contain timings, byte counts, chunk sizes and a character count —
  // never the spoken text — so a dump of this route cannot leak what was said.
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-tts/pcm-telemetry',
    handler: async (req, res) => {
      if (typeof pcmTransport !== 'object' || pcmTransport === null) {
        writeJson(res, 503, { ok: false, error: { code: 'unavailable', message: 'PCM transport is not wired' } })
        return
      }
      if (req.method === 'GET') {
        writeJson(res, 200, { ok: true, records: pcmTransport.snapshot() })
        return
      }
      if (req.method === 'DELETE') {
        if (!isTrustedSettingsRequest(req)) {
        ...
        writeJson(res, 200, { ok: true, cleared: pcmTransport.clear() })
        return
      }
      if (req.method !== 'POST') {
        writeJson(res, 405, { ok: false, error: { code: 'method', message: 'GET, POST or DELETE' } })
        return
      }
      const payload = JSON.parse((await readBody(req, 64 * 1024)).toString('utf8') || '{}')
      // WAV-path marks are keyed by piece id (there is no stream to sequence);
      // PCM marks are keyed by streamId.
      if (payload && payload.kind === 'wav') {
        const merged = typeof pcmTransport.mergeWavBrowser === 'function'
          ? pcmTransport.mergeWavBrowser(payload)
          : false
        writeJson(res, 200, { ok: true, merged })
        return
      }
      const streamId = payload && payload.streamId ? String(payload.streamId) : ''
      if (!streamId) {
        writeJson(res, 400, { ok: false, error: { code: 'param', message: 'streamId is required' } })
        return
      }
      const merged = pcmTransport.mergeBrowser(streamId, payload)
      writeJson(res, 200, { ok: true, merged })
    },
  }), 'dsh-tts: /pcm-telemetry route')
