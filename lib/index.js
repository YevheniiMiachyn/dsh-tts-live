...
export const Config = z.object({
  ...
  // behaviour exactly; reproducibility is one explicit non-negative seed away.
  ...
  samplingSeed: z.number().default(-1)
    .description('Sampler seed. -1 draws a random seed per request, as upstream did; any non-negative value makes the same text reproducible.'),
})
...
export function apply(ctx, config) {
  ...
  function settle(id, item) {
    ...
    if (at === -1) { enqueue({ ...item, ...pieceMeta(item) }); return }
