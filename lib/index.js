...
export function apply(ctx, config) {
  ...
  function speakPiece(sid, text, cfg, kind, extSignal) {
    if (!text) return
  ...
  function reserve(id, sid) {
    enqueue({ id, kind: 'reserved', sessionId: sid })
  }
    ...
    if (event.type === 'assistant/message') {
      ...
      if (liveOptions(cfg).enabled) {
        ...
        if (settled.mode === 'remainder') {
          if (settled.remainder) {
            const left = cleanText(settled.remainder, cfg)
            for (const piece of splitSentences(left, cfg.sentenceChars)) speakPiece(sid, piece, cfg, activeRole)
          }
          return
        }
      }
